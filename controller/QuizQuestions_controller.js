const controller = require('../model/QuizQuestions_Schema');
...
exports.CreateData = async (req,res) => {
    try{
        ...
        console.log("Data Created Successfully:",err.message);
...
exports.UpdateData = async (req,res) => {
    try{
        ...
        if(!update_data){
            return res.status(404).json({message:"Details not found for update"});
        }
        res.status(200).json({
        ...
        console.log("Data Update Failed:",err.message);
        res.status(500).json({message:"Internal Server Error"});
    }
}
